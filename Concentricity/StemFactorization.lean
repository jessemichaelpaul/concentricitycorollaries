/-
Concentricity/StemFactorization.lean

The C3 analytic engine of an A-section: the slice-regular Weierstrass
factorization facts (`weierstrassE`, `spherePrimary`, `sphereUnit` and
their differentiability / zero characterizations), the two-index
log-derivative machinery (`logDeriv_euler`, `logDeriv_weierstrass`,
`stem_identity_logDeriv` — where an individual Euler index p meets an
individual Weierstrass index n), and the divisor ledger tying the stem to
its residue-ℂ zeros (`sphereZero_complete`, `ledger_meromorphic`,
`ledger_orderAt_zero`, `ledger_residueAt_zero`).

These are the proved analytic possessions consumed by the ζ-machinery
(`ZetaWeierstrass`, `ZetaPole`, `ZetaXiMatch`) and the welds. Value-level
throughout: real-number equalities, stem-zero equations, identities of
complex numbers — no σ₀, no ½, no absolute level.
