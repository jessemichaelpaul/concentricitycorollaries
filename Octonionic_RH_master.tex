...
%          readout first takes pi_0(Int_B R_A). Transitivity consumes the
%          transport-generated identifications and makes this set the singleton
%          {kappa}; only then is the inherited read \bar L_A applied to kappa,
%          producing c in R. Hence the residue-C
...
The residue-$\CC$ inverse-image fibres and their inherited transports assemble
inside $\mathcal T_A$ as the semantic value system $\mathcal R_A$, with fully
faithful inclusion $\int\iota_A:\mathcal R_A\hookrightarrow\mathcal T_A$.
For two arbitrary objects $P=(a,x)$ and $Q=(b,y)$ of $\mathcal R_A$, the
base entries $a$ and $b$ are their unique GPV real fibres. The existing local
continuation supplies
\[
 a\xrightarrow{\,h_1^{-1}\,}p_A\xrightarrow{\,h_2\,}b,
 \qquad h_1:p_A\to a,\quad h_2:p_A\to b.
\]
The ordinary target-fibre action $\alpha=(\mu,\gamma)$ then gives the total
morphism $P\to Q$. Hence $\mathcal R_A$ is transitive, therefore connected.
The construction carries the real structure of the residue outputs into this
value system. Transitivity is applied first: connected components consume all
morphism identifications and give $\pi_0(\mathcal R_A)=\{\kappa\}$. Only after
this collapse is the inherited component read
$\overline L_A:\pi_0(\mathcal R_A)\to\RR$ applied to the sole class $\kappa$.
Writing $c=\overline L_A(\kappa)$, its certified evaluation on
the residue states says that every residue-$\CC$ zero $6$-sphere has centre
$c$. Thus the readout has the exact order
$\mathcal R_A\to\pi_0(\mathcal R_A)=\{\kappa\}\to\RR$. The theorem asserts
...
The final categorical step is short. The residue total is transitive, so
$\pi_0$ first consumes its morphisms and produces the singleton $\{\kappa\}$.
The inherited map $\overline L_A$ is then applied to that sole class, giving
$c=\overline L_A(\kappa)\in\RR$. Its evaluation on the semantic residue states
identifies $c$ with every sphere centre. The work lies in constructing the
actual output system, its transports, and that post-component evaluation from
the hypotheses before applying this connectedness argument. The Grothendieck
construction assembles the fibres; no further colimit computation is required
for the readout.
...
Its connected-components set is therefore a singleton $\{\kappa\}$. The
inherited post-component map $\overline L_A$ then reads $\kappa$ as one centre
$c\in\RR$. The concentricity proof takes place in this residue-$\CC$ inverse-image
...
proof developed here runs through ordinary connected components: the
hypotheses assemble the A-section-specific slice-preserving total, its inverse
residue-$\CC$ system is transitive, transitivity makes it connected, and
connectedness makes $\pi_0$ the singleton $\{\kappa\}$. This collapse occurs
before the real read. The unique real GPV face supplies the inherited map
$\overline L_A:\pi_0(\int_{\mathcal B}\mathcal R_A)\to\RR$ and its evaluation
on the represented residue states; applying it to $\kappa$ reads the one real
number. The Grothendieck-components theorem still identifies this singleton
with $\operatorname*{colim}_{\mathcal B}(\pi_0\circ\mathcal R_A)$, but that
colimit presentation is background rather than the route of the final readout.
...
$\operatorname{cayleyProjective}$ is defined on every invertible real
projective matrix after complexification. In the A-section construction it
is applied to the matrices supplied by the projective and GPV actions.
...
Every later appearance of $\operatorname{cayleyProjective}$ is this same
fixed Cayley change of chart applied to a matrix already supplied by the
relevant action. Thus
...
\end{tikzcd}
\]
The top row records the exponential value of the logarithmic lift; the
bottom row records its induced M\"obius map. The square commutes at every
instant. The following lemma records how that map acts on a separately
supplied slice point.

\begin{lemma}[The GPV value and its slice action]\label{lem:gpv-value-action}
\lean{ASection.decodedSliceMobius_diskExpAction,
ASection.sliceMatrixActionArrow_realize,
ASection.C2GpvPackage.branch_exponentialMatrix_eq,
ASection.C2GpvPackage.branch_diskAction_eq}
\uses{def:cayley-disk, thm:slice-exp, thm:log-manifold}
Fix a complex slice, a logarithm $\lambda\in\CC$, and a normalized input
$u\in\Cstar$. Put $v=e^\lambda$ and
$w=C^{-1}\mathbin{\cdot}u$. Then
\[
 M(\lambda)=\operatorname{diag}(v,1),\qquad
 C^{-1}\mathbin{\cdot}
   \bigl(D(\lambda)\mathbin{\cdot}u\bigr)=vw.
\]
Every logarithm of this same value $v$ gives the same $M$, the same $D$,
and the same output on $u$.
\end{lemma}

\begin{proof}
In the fixed complex slice the logarithms of $v$ are
$\lambda+2\pi ik$, $k\in\ZZ$. Exponentiation gives $v$ for each of them,
so their diagonal matrices and Cayley conjugates agree. The action formula
follows from $D(\lambda)=CM(\lambda)C^{-1}$:
\[
 C^{-1}\mathbin{\cdot}\bigl(D(\lambda)\mathbin{\cdot}u\bigr)
   =M(\lambda)\mathbin{\cdot}w=vw.
\]
\end{proof}

Here $\lambda$ is the logarithmic argument, $v$ is its exponential value,
and $u$ is the normalized input on which the resulting matrix acts. The
real GPV level is $\operatorname{Re}\lambda=\log|v|$; the full package also
retains the direction and winding of the logarithmic preimage.
...
\begin{proposition}[The global C2/GPV package]\label{prop:c2-gpv-package}
\lean{ASection.C2GpvPackage.exp_canonicalLift,
ASection.C2GpvPackage.canonicalLift_eq_primePower,
ASection.C2GpvPackage.branch_re_eq,
ASection.C2GpvPackage.branch_winding,
ASection.C2GpvPackage.branch_realPreimage_eq,
ASection.C2GpvPackage.branch_exponentialMatrix_eq,
ASection.C2GpvPackage.branch_diskAction_eq,
ASection.C2GpvPackage.branch_sliceActionHom_eq}
\uses{def:A-section, thm:log-manifold, lem:gpv-value-action}
For $z$ in the C2 half-space put
\[
 \Gamma_A^{\mathrm{C2}}(z)=\sum_p\ell_p(z),\qquad
 v_A^{\mathrm{C2}}(z)=A(z)=\exp\Gamma_A^{\mathrm{C2}}(z),
\]
and let
\[
 \beta^{\mathrm{C2}}_{A,z}:=
 \bigl(\operatorname{Re}\Gamma_A^{\mathrm{C2}}(z):
       \operatorname{OnePoint}(\RR)\bigr)
 \in\operatorname{Ob}(\mathcal B).
                                                        \tag{$\beta$}\label{eq:gpv-base-object}
\]
The prime sum, its exponential value, this unique real preimage, the
direction and winding data of its logarithmic fibre, the branch-independent
matrices $M(\Gamma_A^{\mathrm{C2}}(z))$ and
$D(\Gamma_A^{\mathrm{C2}}(z))$, and their slice action form the global
C2-instantiated GPV package.
\end{proposition}

\begin{proof}
Condition \textup{C2} supplies the completed prime-power sum and its
exponential identity. If $\lambda$ is any logarithmic branch of $A(z)$,
then
$e^{\operatorname{Re}\lambda}=|A(z)|$, so
$\operatorname{Re}\lambda=\log|A(z)|$. All branches therefore determine
the displayed real object of the existing base $\mathcal B$. In each
complex slice they differ by whole windings; the logarithm manifold retains
their direction and winding data. Proposition~\ref{prop:winding-signature}
and Lemma~\ref{lem:gpv-value-action} give agreement of their exponential
matrices and of the resulting action on every supplied input.
\end{proof}
...
ASection.exists_euler_to_pole_with_initial}
\uses{thm:winding-lift, prop:winding-signature, def:A-section}
Let $z_0,z_1\in\operatorname{OnePoint}(\RR)$ label the domain endpoints
of a local continuation. By the $I$-independent lift
(Definition~\ref{def:slice-squares}), evaluating the pole-cancelled factor
$g_A$ off the real axis is evaluating its one holomorphic stem on the one
compactified plane every slice identifies with. A GPV transport
from $z_0$ to $z_1$ records that stem evaluation along a path. It carries
...
evaluated, running from the point of $z_0$ to the point of $z_1$; the value
$v$ the pole-cancelled factor $g_A$ takes there; and a logarithmic lift $\Gamma$, the
continuous logarithm of that value.
The instant parametrizes the transport from $z_0$ to $z_1$ and belongs to the transport alone; the
...
each instant is the continued value. C2 supplies the initial branch at a real
half-space point $z_0$,
\[
  \Gamma^g(0)=\log(z_0-p_A)+\sum_p\ell_p(z_0),
\]
whose exponential is $g_A(z_0)=(z_0-p_A)A(z_0)$. The unique tame lift then fixes
the continuation of this $g_A$ branch along the chosen nonvanishing path.
Thus the argument reads what the path actually carries: the one
pole-cancelled factor from its Euler presentation on the half-space, through
the region where its Weierstrass presentation agrees with it, to its
continued value at the finite pole $p_A$. At every instant its exponential
fibre has one real level. What varies with the instant is the continued
$g_A$ value and its logarithmic lift. The C2 prime-power sum names the
initial A-value component; it is not asserted to remain a C2 formula outside
its half-space. The matrix generated by the continued $g_A$ lift varies
along the path and acts on the standing input. The beginning and arrival of
the path are determined by the hypotheses: it
...
The factor takes its values on the compactified sphere, and the exponential
...
$v^{*}(t)$ on the compactified sphere, the same value seen where the factor takes it. Their defining equations are
\[
\begin{aligned}
 \delta^*(0)&=\operatorname{complexPoint}(z_0),
 &\delta^*(1)&=\operatorname{complexPoint}(z_1),\\
 v^*(t)&=g_A\bigl(\delta^*(t)\bigr),
 &e^{\Gamma(t)}&=v(t),\\
 \Gamma(1)-\Gamma(0)&=2\pi i k,
 &&k\in\ZZ\quad\text{when the value path is a circuit}:
\end{aligned}
\]
the domain path runs from $z_0$ to $z_1$; the value, read on the sphere, is the
continued pole-cancelled factor along it; and the lift exponentiates to the
value where the exponential lives. The last equation records the circuit
case, when the source and target values agree, and $k$ is its winding
integer. At each instant the real GPV object is
$\operatorname{Re}\Gamma(t)=\log|v(t)|$. Thus the domain endpoints label
the analytic path, and the real levels index its GPV packages.
\end{definition}
...
Condition \textup{C2} supplies the canonical global arithmetic instantiation
of the same GPV logarithmic-fibre theorem, but for the original A-value rather
than for the pole-cancelled value tape of the \texttt{GpvTransport} structure
above. Along an Euler half-space loop $\delta$,
\[
...
\subsection*{The C2/GPV package and the direct slice action $A^{\mathrm{slice}}$}

Condition \textup{C2} supplies the complete exponential base for $A$ and
thereby instantiates the global GPV package whose real-preimage datum lies in
the pre-existing action groupoid $\mathcal B$. Separately, conditions
\textup{C1} and \textup{C3} continue the pole-cancelled factor $g_A$ through
$p_A$. This subsection first records that local arrival datum and then the
global slice action carried by the C2 package; the two value functions retain
...
ASection.eulerPrimeSum_eq_primePower}
\uses{def:A-section, def:cayley-disk, lem:disk-homomorphism}
Write $p_A$ for the pole of $A$ and
$g_A=\texttt{distinguishedPoleFactor\,A}$ for the pole-cancelled factor.
Condition \textup{C2} presents this factor in its right half-space and
condition \textup{C3} presents the same factor away from the pole:
\[
\begin{aligned}
 g_A(z)
   &=(z-p_A)
...
Thus every branch at the pole produces the same $D_A$. The Euler and
Weierstrass expressions displayed above are the two presentations of this
same locally continued factor.
\end{definition}

The same construction continues the pole-cancelled factor to the finite
pole. The domain endpoint remains $p_A$, and $A(p_A)=\infty$. The
continuation supplies the A-specific datum carried there: the
finite multiplier $u_A=g_A(p_A)$, its exponential fibre
$\exp^{-1}(u_A)=\{\lambda_A+2\pi ik:k\in\ZZ\}$, the unique real GPV
preimage
\[
  \beta_A:=\bigl(\operatorname{Re}\lambda_A:\operatorname{OnePoint}(\RR)\bigr)
     \in\operatorname{Ob}(\mathcal B),
\]
and the branch-independent disk action $D_A$. The domain endpoint $p_A$ and
the GPV object $\beta_A$ are two roles in the same pre-existing base.
...
\begin{proof}
The value along the run is $g_A(\delta(t))$. Condition \textup{C1} makes
$g_A$ entire after cancellation and gives $g_A(p_A)=u_A\ne0$; hence $g_A$
is not identically zero and its zero set is discrete. Choose the domain path
from the C2 half-space to $p_A$ in the complement of that zero set. Along
this chosen run the value is nonzero, entering where
$g_A=(z-p_A)\,e^{\Gamma_A}$ (condition \textup{C2}) and arriving at
$v(1)=g_A(p_A)=u_A$; condition \textup{C3} presents the same factor off the
pole in Weierstrass form. The arriving action is $D_A$ as
...
The GPV transport $\tau$ follows the pole-cancelled factor $g_A$ along a
domain path and has
\[
 e^{\Gamma^g_\tau(t)}=g_A(\delta_\tau^*(t)),\qquad
 \beta^g_{\tau,t}=
 \bigl(\operatorname{Re}\Gamma^g_\tau(t):
       \operatorname{OnePoint}(\RR)\bigr)
 \in\operatorname{Ob}(\mathcal B).
\]
It is the local C1/C3 continuation layer. At a real C2 starting point $a$
its initial lift is
\[
 \Gamma^g_\tau(0)=\log(a-p_A)+\Gamma_A^{\mathrm{C2}}(a),
\]
because $g_A(a)=(a-p_A)A(a)$; at the finite pole it produces $u_A$,
$\beta_A$, and $D_A$. The starting levels consequently satisfy
\[
 \operatorname{Re}\Gamma^g_\tau(0)
   =\log|a-p_A|+\operatorname{Re}\Gamma_A^{\mathrm{C2}}(a).
\]
The domain path $\delta_\tau^*$ describes the analytic continuation, while
$\beta^g_{\tau,t}$ gives its real GPV object at each instant. C1/C3 supply
this local specialization for the later continuation through $p_A$.

Condition \textup{C4}, infinitely many residue-$\CC$ zeros, makes the
semantic C-residue locus infinite and therefore nonempty. Its morphisms are
the matrix actions and $G_2$ automorphisms already carried by the action
groupoids.

The C2/GPV construction supplies an object of the existing base and one
branch-independent Möbius element together with its direction and winding
data. The sphere object is supplied by the normalized compactified input: a
state $(I,u)$ names the actual slice Riemann sphere $\CC_I^{*}$ and a point
on it. The Part~2 GPV uniqueness theorem makes the exponentiated matrix
branch-independent; that matrix is the action arrow on the actual sphere.

\begin{lemma}[Orbit--stabilizer on an actual slice sphere]\label{lem:orbit-stab}
\lean{GreatCircle.actualSphereOrbitEquivQuotientStabilizer,
GreatCircle.actualSphereOrbitEquivQuotientStabilizer_mk,
GreatCircle.actualSphere_moebius_transitive,
MulAction.orbitEquivQuotientStabilizer}
\uses{def:two-worlds, lem:mobius-transitive}
For $I\in S^6$ and $q\in\CC_I^{*}$, the Möbius matrix action has
stabilizer
\[
  \operatorname{Stab}(q)
  =\{m\in\operatorname{M\ddot ob}(\CC^{*}):m\mathbin{\cdot}q=q\},
\]
and the canonical orbit map induces a bijection
\[
 \operatorname{M\ddot ob}(\CC^{*})/\operatorname{Stab}(q)
   \;\xrightarrow{\ \cong\ }\;
 \operatorname{M\ddot ob}(\CC^{*})\mathbin{\cdot}q
   =\CC_I^{*},
 \qquad m\operatorname{Stab}(q)\longmapsto m\mathbin{\cdot}q.
\]
Thus an arrow of the slice action groupoid is the actual matrix action
$m:q\to m\cdot q$. Orbit--stabilizer supplies the intrinsic description of
the actual sphere as the orbit and of its action arrows as stabilizer cosets.
\end{lemma}

\begin{proof}
The action laws make $\operatorname{Stab}(q)$ a subgroup. The displayed map
is well defined because right multiplication by an element fixing $q$ does
not change $m\cdot q$; equality of two images says that the corresponding
matrices differ by such a stabilizer element. Surjectivity is the definition
of the orbit, and Lemma~\ref{lem:mobius-transitive} identifies that orbit
with the whole actual sphere $\CC_I^{*}$.
\end{proof}

\begin{definition}[The full C2/GPV slice action
$A^{\mathrm{slice}}$]\label{def:aslice}
\lean{ASection.sliceMatrixAction, ASection.sliceMatrixActionArrow,
ASection.sliceExponentialEnd,
ASection.C2GpvPackage.realPreimage,
ASection.C2GpvPackage.canonicalLift_eq_primePower,
ASection.C2GpvPackage.branch_realPreimage_eq,
ASection.C2GpvPackage.branch_diskAction_eq,
ASection.C2GpvPackage.sliceActionHom,
ASection.C2GpvPackage.sliceActionHom_eq_map,
ASection.C2GpvPackage.sliceActionHom_realize,
ASection.C2GpvPackage.sliceActionHom_natural_dir}
\uses{def:cayley-disk, prop:c2-gpv-package, lem:gpv-value-action,
lem:orbit-stab}
Let $(I,u)$ be the normalized input supplied by the compactified
octonionic input interface. Its physical point is
\[
 q_{I,u}=\varphi_I\!\left(C^{-1}\mathbin{\cdot}u\right)
       \in\CC_I^{*}\subset\OO^{*}.
\]
For $z$ in the C2 half-space let $\mathcal P_{A,z}$ be the full GPV package
of Proposition~\ref{prop:c2-gpv-package}. It carries the real base object
$\beta^{\mathrm{C2}}_{A,z}$, direction and winding data, and the
branch-independent matrices
\[
\begin{aligned}
 M_{A,z}
   &=M\bigl(\Gamma_A^{\mathrm{C2}}(z)\bigr)
     =\operatorname{diag}\bigl(A(z),1\bigr),\\
 D_{A,z}
   &=D\bigl(\Gamma_A^{\mathrm{C2}}(z)\bigr)
     =C\,M_{A,z}\,C^{-1}.
\end{aligned}
\]
The action $A^{\mathrm{slice}}$ uses $D_{A,z}$ on the stored normalized
coordinate $u$. On the analytic chart of the actual sphere this same action
has Möbius leg $C^{-1}D_{A,z}C=M_{A,z}$ and gives the genuine
$\SphereWorld$ arrow
\[
 A^{\mathrm{slice}}_A(\mathcal P_{A,z};I)
   =(1,M_{A,z}):\CC_I^{*}\longrightarrow\CC_I^{*},
 \qquad
 q_{I,u}\longmapsto q_{I,D_{A,z}\cdot u}.
                                                        \tag{Slice}\label{eq:full-c2-slice-action}
\]
The normalized input supplies the actual sphere and point; the C2/GPV
package supplies the unique real fibre, the full logarithmic data, and the
matrix action. Orbit--stabilizer identifies the resulting action-groupoid
arrow intrinsically on that actual sphere.

At the finite pole the local C1/C3 transport supplies the specialization
\[
 D(\Gamma^g_\tau(1))=D_A,\qquad
 (1,C^{-1}D_A C):\CC_I^*\longrightarrow\CC_I^*,
\]
by Lemma~\ref{lem:finite-pole-arrival}. This is the continuation action used
later in the passage through $p_A$.
\end{definition}

\begin{proposition}[The slice action is functorial]\label{prop:aslice-functor}
\lean{ASection.sliceMatrixAction,
ASection.sliceMatrixAction_realize,
ASection.sliceMatrixActionArrow_realize_eq_orbitStabilizer,
ASection.sliceExponentialEnd,
ASection.sliceExponentialEnd_add,
ASection.sliceExponentialEnd_comp,
ASection.C2GpvPackage.branch_sliceActionHom_eq,
ASection.C2GpvPackage.sliceActionHom_comp,
ASection.C2GpvPackage.sliceActionHom_natural_dir}
\uses{def:aslice, lem:disk-homomorphism, lem:orbit-stab}
For every actual slice sphere $\CC_I^{*}$, the ordinary matrix action is a
functor
\[
 \operatorname{M\ddot ob}(\CC^{*})\ltimes\operatorname{OnePoint}(\CC)
   \longrightarrow\SphereWorld,
 \qquad
 (m:u\to m\cdot u)\longmapsto
 (1,C^{-1}mC):\CC_I^{*}\to\CC_I^{*}.
                                                        \tag{Act}\label{eq:slice-action-functor}
\]
The exponential assignment is a homomorphism into the endomorphisms of
$\CC_I^{*}$. Restricting this action to the matrices furnished by the
C2/GPV packages defines $A^{\mathrm{slice}}$: it preserves identity and
composition, is independent of the logarithmic branch by GPV uniqueness,
and commutes with the $G_2$ direction action. Orbit--stabilizer supplies the
object-and-arrow description of this action on the whole actual sphere.
\end{proposition}

\begin{proof}
An object of the action groupoid is the supplied point $u$, and a morphism
$m:u\to m\cdot u$ carries its source to its stated target. Realizing $m$ in
the analytic chart sends
\[
 q_{I,u}\longmapsto q_{I,m\cdot u}.
\]
The identity and composition laws are the group-action laws. For the
exponential matrices they read
\[
 D(0)=1,\qquad D(z+w)=D(z)D(w),
\]
and the conjugated analytic-chart action obeys
\[
 C^{-1}(mn)C=(C^{-1}mC)(C^{-1}nC).
\]
Theorem~\ref{lem:orbit-stab} identifies this realized action with the
orbit--stabilizer image of the matrix coset at the same actual point.

If two branches belong to one C2/GPV package, GPV uniqueness gives the same
real fibre and exponentiation gives the same matrix, so they induce the same
action arrow. Finally, for $\gamma\in G_2$ and any matrix $m$, the sphere
arrows satisfy
\[
 (1,C^{-1}mC)\fatsemi(\gamma,1)
   =(\gamma,C^{-1}mC)
   =(\gamma,1)\fatsemi(1,C^{-1}mC).
\]
Thus the matrix changes the point inside its actual sphere, $G_2$ changes
the direction, and the two actions commute. These are precisely the
identity, composition, orbit--stabilizer, branch-independence, and
direction-coherence laws of $A^{\mathrm{slice}}$.
\end{proof}

$A^{\mathrm{slice}}$ now supplies the hypothesis-derived matrix action on
each normalized input. The A-section diagram $\mathcal A_A$ packages the
input, its matrix-acted point, and the value obtained by evaluating the
original section there; its Grothendieck assembly is $\mathcal T_A$.
...
acts on the supplied slice points, and
$A_{\OO}:\mathcal H_1\to\mathcal H_1$ gives the original A-section value
at each acted point. The functor this subsection builds lands in
$\mathbf{Grpd}$ itself:
\[
...
(\texttt{projectiveArrowHom}). The C2 exponential matrix and GPV uniqueness
make this slice-preserving action well defined on each supplied sphere;
the local C1/C3 continuation through $p_A$ remains available for the later
transitivity argument. The
...
\[
  ...
  A_{\OO}\bigl(I,A^{\mathrm{slice}}(b)\cdot u\bigr)\,\Bigr):
\]
a slice direction $I\in S^6$ with a normalized input point $u$, the point
obtained by the matrix action $A^{\mathrm{slice}}(b)$, and the value of
$A_{\OO}$ at that acted point. Only the input is free. The group action
determines the middle entry, and $A_{\OO}$ determines the third entry from
that same point. The second and third entries are one point of the
equivariant realization's graph. The middle entry lies on its actual slice
Riemann sphere and the third entry lies in $\OO^{*}$.
...
\begin{aligned}
  &\Bigl(\,(I,u),\ \
    ...
    A_{\OO}\bigl(I,A^{\mathrm{slice}}(a)\cdot u\bigr)\,\Bigr)\\
  &\quad\longmapsto
  \Bigl(\,\bigl(I,\
      A^{\mathrm{slice}}(b)^{-1}\mathbin{\cdot}
      \bigl(A^{\mathrm{slice}}(h)\mathbin{\cdot}
...
\end{aligned}
\]
The third entry is $A_{\OO}$ applied to that same target middle point.
The matrices acting here are those of $A^{\mathrm{slice}}$, constructed
from the C2 exponential action; the C1/C3 continuation supplies the local
specialization through $p_A$.
...
at $b$. The Concentricity Theorem applies its inherited real read only after
the semantic residue total has been collapsed by $\pi_0$; its evaluation
on a residue state gives that state's sphere centre.
...
$A^{\mathrm{slice}}$; and every value entry is $A_{\OO}$ applied to the
acted point. The total is the A-section's graph made one category: the
A-specific matrix actions move its states between fibres, and
\textup{C3--C4} supply the semantic residue-$\CC$ zero-sphere states
selected below. The total has its canonical projection to the base,
$(b,x)\mapsto b$ and $(h,\gamma)\mapsto h$, which forgets the state
\textup{(}\texttt{CategoryTheory.Grothendieck.forget}\textup{)}; over it
the total holds one fibre per point of the real great circle. The components diagram
$\pi_0\circ\mathcal A_A:\mathcal B\to\mathbf{Set}$ sends each base object to
the set of components of its fibre. Lemma~\ref{lem:pi0-grothendieck}
identifies its colimit with $\pi_0$ of the total. This is useful categorical
background; the theorem itself first collapses the residue total to its sole
component and only then applies its inherited real read.
...
$6$-sphere, a single connected component of the octonionic world. The
categorical readout is Corollary~\ref{cor:direct-connected-readout}: first
$\pi_0$ collapses the connected residue total to $\{\kappa\}$, and then its
inherited map to $\RR$ is evaluated at $\kappa$. The component-colimit lemmas
are retained as general background. With these in hand
...
Let $J$ be a nonempty connected category and let $X$ be a set. Then there is
a unique class $\kappa\in\pi_0(J)$ and, for every map
$\overline L:\pi_0(J)\to X$, there is a unique $c\in X$ such that
$\overline L(\xi)=c$ for every $\xi\in\pi_0(J)$; namely
$c=\overline L(\kappa)$. Thus the order of the direct readout is
\[
  J\xrightarrow{\ \pi_0\ }\pi_0(J)=\{\kappa\}
    \xrightarrow{\ \overline L\ }X.
\]
The first arrow consumes the morphism-generated identifications. The second
arrow is applied only to the resulting sole component.
\end{corollary}
\begin{proof}
By the connected-components criterion of \cite[\S8.3,
Rem.~8.3.5]{Riehl}, nonemptiness and connectedness say exactly that
$\pi_0(J)$ is a singleton. Let $\kappa$ be its sole element. Every
$\xi\in\pi_0(J)$ is therefore $\kappa$, so every value of $\overline L$ is
$\overline L(\kappa)$. Taking $c=\overline L(\kappa)$ proves existence, and
evaluation at $\kappa$ proves uniqueness.
\end{proof}
...
carrying their real structure into the semantic residue value system
$\mathcal R_A$ inside $\mathcal T_A$. Transitivity makes this system
connected, so $\pi_0(\mathcal R_A)=\{\kappa\}$. Only after that collapse, the
inherited map $\overline L_A:\pi_0(\mathcal R_A)\to\RR$ is applied to
$\kappa$. Its evaluation
on each represented semantic residue state identifies
$\overline L_A(\kappa)$ with that sphere's centre.
...
The functor $A^{\mathrm{slice}}$ acts on the supplied point of its actual
slice sphere, and $A_{\OO}$ acts on the resulting point. Thus an object of
the fibre $\mathcal A_A(b)$ is the already constructed triple
\[
 x=\Bigl((I,u),\ \bigl(I,A^{\mathrm{slice}}(b)\mathbin{\cdot}u\bigr),\
 A_{\OO}\bigl(I,A^{\mathrm{slice}}(b)\mathbin{\cdot}u\bigr)\Bigr).
...
form. Evaluation at $p_A$ gives the invertible multiplier and the local
continuation action:
\[
\begin{aligned}
...
the logarithmic branch changes only the winding. The C2 exponential action
defines $A^{\mathrm{slice}}$ and the action diagram $\mathcal A_A$; the
displayed C1/C3 data supply their local continuation through $p_A$ for
the later transitivity argument. The real GPV structure carried by the
action construction supplies the post-component map and its
state-evaluation theorem.
After residue-total transitivity, $\pi_0$ is taken first and that map is
evaluated on its sole class. The residue restriction begins with the semantic
...
\]
Its points in the Cayley action chart are
\[
 C\mathbin{\cdot}
...
infinite. The positive-imaginary condition chooses one analytic
representative of each zero sphere. In every fibre, the residue condition
selects the existing states whose middle points lie on these C-residue
zero spheres. Their third entries are the values of $A_{\OO}$ at those
same points.

\emph{The objects of the residue fibre.}
Fix a base object $a$. The semantic inverse image gives the object
collection
\[
 \operatorname{Ob}\mathcal R_A(a):=
 \bigl\{x\in\operatorname{Ob}\mathcal A_A(a):
   \text{the middle point of $x$ lies on a C-residue zero sphere of $A$}
 \bigr\}.
                                                        \tag{O}\label{eq:residue-fibre-objects}
\]
This selects states by the C-residue locus of the original A-section
inside the existing action diagram. The direction and acted point remain
the first two entries of the same state triple; its value is the third.
...
\]
The A-specific matrix action carries the selected middle point of $x$
to the middle point of $\mathcal A_A(h_2)(x)$, while $A_{\OO}$ acts on
that same target point. The inherited action on the semantic C-residue
states therefore gives the stated restriction, and the target state
satisfies \eqref{eq:residue-fibre-objects}.
...
$\mathcal A_A$, after restricting its objects. The restricted-action
assignment in Definition~\ref{def:residue-subdiagram} shows that every
...
For an A-section $A$, the assembled semantic C-residue value system
$\mathcal R_A$ inside $\mathcal T_A$ is transitive: any two of its objects
are joined by a morphism.
\end{lemma}
\begin{proof}
Let
\[
 P=(a,x),\qquad Q=(b,y)
\]
be arbitrary objects of $\mathcal R_A$. Their base entries $a$ and $b$ are
the unique GPV real fibres carried by those two states. The existing C1/C3
continuation through the finite pole supplies legs
\[
 h_1:p_A\longrightarrow a,\qquad h_2:p_A\longrightarrow b.
\]
Invert the first leg and compose in the existing action groupoid:
\[
 a\xrightarrow{\,h_1^{-1}\,}p_A
   \xrightarrow{\,h_2\,}b,
 \qquad h:=h_1^{-1}\fatsemi h_2:a\longrightarrow b.
                                                        \tag{Pole}\label{eq:through-pole-base-route}
\]

Apply the inherited A-specific transport to $x$. In the target fibre at
$b$, the ordinary sphere-world action supplies an arrow
\[
 \alpha=(\mu,\gamma):\mathcal R_A(h)(x)\longrightarrow y.
\]
Here the ordinary real M\"obius action $\mu$ moves the first, normalized
input register. The commuting action square carries the middle register by
$A^{\mathrm{slice}}$, and the $G_2$ component $\gamma$ together with the
equivariance of $A_{\OO}$ carries the original-value register. M\"obius
transitivity supplies $\mu$ on the normalized input and $G_2$-transitivity
supplies $\gamma$ on the slice direction; their commutation makes this the
displayed joint sphere-world arrow. Since both source and target are selected
residue states, the full inclusion of Lemma~\ref{lem:iota-full-faithful}
retains this same arrow in the residue fibre. Therefore the Grothendieck pair
\[
 (h,\alpha):(a,x)\longrightarrow(b,y),
 \qquad h=h_1^{-1}\fatsemi h_2,
                                                        \tag{Tot}\label{eq:through-pole-total-route}
\]
is a morphism of $\mathcal R_A$.

The endpoint states were arbitrary, so $\mathcal R_A$ is transitive. The
two-leg continuation in \eqref{eq:through-pole-base-route} is the base
composition through $p_A$, while $\alpha$ is the ordinary fibre arrow at the
target $b$. The common centre enters only after the resulting total morphisms
have been consumed by $\pi_0(\mathcal R_A)$.
\end{proof}
...
$\mathcal R_A$ assembled inside $\mathcal T_A$. Its connected-components set is a
...
\begin{gathered}
 ...
 \bigl(a\longmapsto\mathcal R_A(a)\bigr):
   \mathcal B\longrightarrow\mathbf{Grpd},
 \qquad
 \mathcal R_A=\displaystyle\int_{a\in\mathcal B}\mathcal R_A(a)
...
\]
The already-proved functor $A^{\mathrm{slice}}$ is the C2/GPV exponential
matrix action on the supplied actual slice spheres. Its matrices act on
points, and their products give the action on composites. The displayed
C1/C3 action is the local continuation through $p_A$ used below.
The functor $A_{\OO}$ evaluates each acted point equivariantly, and the
action diagram $\mathcal A_A$ combines these two functors in the triples
\[
 \Bigl((I,u),\ \bigl(I,A^{\mathrm{slice}}(b)\mathbin{\cdot}u\bigr),\
 A_{\OO}\bigl(I,A^{\mathrm{slice}}(b)\mathbin{\cdot}u\bigr)\Bigr),
\]
whose Grothendieck total is $\mathcal T_A$. The local GPV lift at $p_A$
has the exponential fibre
\[
 \exp^{-1}(u_A)=\{\lambda_A+2\pi ik:k\in\ZZ\},
\]
and every member of that fibre has the same real part
$\operatorname{Re}\lambda_A$ (Remark~\ref{rmk:gpv-real-fibre}). The C2
GPV real fibre and its matrix action are carried through
$A^{\mathrm{slice}}$, $\mathcal A_A$, and $\mathcal T_A$ before the
residue restriction. The C1/C3 continuation supplies the passage through
$p_A$ in the transitivity proof. The inherited component map is evaluated
on residue states only after transitivity has collapsed the residue total
under $\pi_0$.
...
\]
Its objects are precisely the semantic C-residue zero-sphere states
selected in $\mathcal T_A$, and its morphisms are the projective/M\"obius and
$G_2$ arrows already constructed in $\mathcal A_A$.

Lemma~\ref{lem:c-residue-transitive} applies the existing local continuation
to two residue states $P=(a,x)$ and $Q=(b,y)$. Their base entries $a$ and
$b$ are the unique GPV real fibres carried by those states. With
$h_1:p_A\to a$ and $h_2:p_A\to b$, the base component is
\[
  a\xrightarrow{h_1^{-1}}p_A\xrightarrow{h_2}b,
  \qquad h=h_1^{-1}\fatsemi h_2.
\]
At the target, the ordinary fibre arrow
$\alpha=(\mu,\gamma):\mathcal R_A(h)(x)\to y$ moves the normalized input
by the real M\"obius action, the middle register by $A^{\mathrm{slice}}$,
and the original-value register by the $G_2$ action and equivariance. Thus
$(h,\alpha):P\to Q$, so $\mathcal R_A$ is a transitive groupoid and
therefore a connected category.

Condition \textup{C4} supplies objects, so transitivity gives
\[
  \pi_0(\mathcal R_A)=\{\kappa\}.
                                                        \tag{$\pi$}\label{eq:residue-total-singleton}
\]
At this point the morphisms have already been consumed by $\pi_0$. The
inherited read has the component set as its domain:
\[
  \mathcal R_A\xrightarrow{\ \pi_0\ }\pi_0(\mathcal R_A)=\{\kappa\}
       \xrightarrow{\ \overline L_A\ }\RR.
                                                        \tag{Read}\label{eq:residue-readout-chain}
\]
Set $c=\overline L_A(\kappa)$. Every semantic residue-zero state represents
$\kappa$, and the state-evaluation property of the inherited GPV real face
identifies its sphere centre with $\overline L_A(\kappa)$. Thus $c$ is the
common centre of all the residue-$\CC$ zero spheres. Condition \textup{C4}
supplies infinitely many such spheres.
