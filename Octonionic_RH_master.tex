...
and that is the entry the sweep produced. What $\mathcal A_A$ does is sweep the
slice: the direction $I$ ranges over $S^6$, $D_A$ positions the coordinate in
the slice that direction selects, and $A_{\OO}$ realizes the value there. $G_2$
...
image is taken over exactly what the sweep produces, so transitivity on the
