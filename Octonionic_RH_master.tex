...
$x_N,y_N$ with the authored C3 coordinates $z_i$, and the graph equation then
pulls each one back uniquely through the north action:
\[
...
run at its own input.  By the inverse-image definition they differ in nothing
but those inputs, and the inputs lie in the C-residue zero locus.

The rectangles used below are therefore the functorial arrows
\[
  \mathcal A_A(\operatorname{eulerToNorth}_1),\qquad
  \mathcal A_A(\operatorname{eulerToNorth}_2),
\]
both running on the one fixed tape from the common projective zero frame to the
one north frame.  Write
\[
  \operatorname{eulerToNorth}_1,\ \operatorname{eulerToNorth}_2:0\longrightarrow N
\]
...
\]
and \texttt{stabilizerPart\_unique} says that any north-stabilizer element
giving this same factorization equals $r_i$.
...
\]
along the bottom the arrow elements $a_A(\operatorname{eulerToNorth}_1)$ and $a_A(\operatorname{eulerToNorth}_2)$.  Read at $x=0$,
$y=N$, \eqref{eq:orbit-stabilizer} says that each square commutes:
\[
  a_A(\operatorname{eulerToNorth}_1)\,a_A(0)=D_A\,R_1,\qquad
  a_A(\operatorname{eulerToNorth}_2)\,a_A(0)=D_A\,R_2.       \tag{S}\label{eq:north-face-squares}
\]
What $\mathcal A_A(\operatorname{eulerToNorth}_1)$ and $\mathcal A_A(\operatorname{eulerToNorth}_2)$ contribute is that they carry both horizontal
...
\[
  a_A(\operatorname{eulerToNorth}_1)\mathbin{\cdot}\bigl(a_A(0)\mathbin{\cdot}u_\ast\bigr)=z_1,\qquad
  a_A(\operatorname{eulerToNorth}_2)\mathbin{\cdot}\bigl(a_A(0)\mathbin{\cdot}u_\ast\bigr)=z_2,
...
u_1 \arrow[d,"D_A"]\\
a_A(0)\mathbin{\cdot}u_\ast \arrow[r,"a_A(\operatorname{eulerToNorth}_1)"']\&
z_1
\end{tikzcd}
\]
\[
...
u_2 \arrow[d,"D_A"]\\
a_A(0)\mathbin{\cdot}u_\ast \arrow[r,"a_A(\operatorname{eulerToNorth}_2)"']\&
z_2
\end{tikzcd}
\]
...
\begin{aligned}
 D_A\mathbin{\cdot}\bigl(\operatorname{cayleyProjective}(r_1)\mathbin{\cdot}u_\ast\bigr)
   &=a_A(\operatorname{eulerToNorth}_1)\mathbin{\cdot}\bigl(a_A(0)\mathbin{\cdot}u_\ast\bigr)
    =z_1=D_A\mathbin{\cdot}u_1,\\
 D_A\mathbin{\cdot}\bigl(\operatorname{cayleyProjective}(r_2)\mathbin{\cdot}u_\ast\bigr)
   &=a_A(\operatorname{eulerToNorth}_2)\mathbin{\cdot}\bigl(a_A(0)\mathbin{\cdot}u_\ast\bigr)
...
\begin{aligned}
 ...
 &=\operatorname{cayleyProjective}(r_2r_1^{-1})\,
    \operatorname{cayleyProjective}(r_1)\mathbin{\cdot}u_\ast\\
 &=\operatorname{cayleyProjective}(r_2)\mathbin{\cdot}u_\ast=u_2,\\[1ex]
 D_A\,\operatorname{cayleyProjective}(r)\,D_A^{-1}\mathbin{\cdot}z_1
 &=D_A\mathbin{\cdot}\bigl(\operatorname{cayleyProjective}(r)
     \mathbin{\cdot}u_1\bigr)\\
...
\[
  k=\operatorname{eulerToNorth}_1^{-1}
    \fatsemi \operatorname{eulerToNorth}_2
    \;:\;N\longrightarrow N.
\]
...
\[
  ...
  =\operatorname{stabilizerPart}(\operatorname{eulerToNorth}_2)
     \operatorname{stabilizerPart}(\operatorname{eulerToNorth}_1)^{-1}
  =r_2r_1^{-1}=r.                                  \tag{R}\label{eq:north-relative-stabilizer}
\]
--- equivalently, from the unique factorizations $\operatorname{eulerToNorth}_i=o_Nr_i o_0^{-1}$,
