...
\item \emph{Infinite Weierstrass factorization.} Over the \emph{full divisor} of $A$ ---
  ...
  \cite{GV}\textup{)}; the factorization holds with either family infinite. \textup{(}The left
...
\uses{lem:pi0-grothendieck, eq:placement-set}
The single-component conclusion also follows from Quillen's \S1 \cite{Quillen73}, on the same
assembly, in either of two forms. \emph{Finality (post-placement reading --- this half
consumes the placement \eqref{eq:placement-set}):} after the placement, the zero-bearing part
of $\mathcal B$ is a \emph{single} level; by \textup{C1} the continuation of $A$ carries it to
the north pole $\nfr$, and the winding lift supplies a unique tame companion along the
continuation (\cite[Def.~4.20, Cor.~5.13]{GPVwind}); so the comma category from that level to
$\nfr$ is non-empty and connected, and $\nfr$ is final over the zero-bearing part of
$\mathcal B$ --- the $\pi_0$ shadow of Theorem~A (``if $Y\backslash f$ is contractible for
every $Y$, then $f$ is a homotopy equivalence''; connected and non-empty is the
$\pi_0$-level hypothesis; textbook form Riehl \cite[\S8.3, \S8.5]{Riehl}). Absent the
placement the static base has no arrows between distinct levels, and this half asserts
nothing.
