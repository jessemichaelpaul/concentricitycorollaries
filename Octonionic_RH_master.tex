...
It sends a base object $b$ to the groupoid of normalized A-section states
positioned by the object frame
$a_A(b)=\operatorname{cayleyProjective}(o_b)D_A$. An object of that groupoid is
one constrained graph \eqref{eq:north-graph}: a slice direction $I\in S^6$ and
a normalized input coordinate $u$, together with the positioned coordinate
$a_A(b)\mathbin{\cdot}(I,u)$ that the frame determines and the A-value realized
there. An arrow of that groupoid is a single element $\gamma\in G_2$, acting on
the slice direction and carrying the rest of the graph with it, so the fibre is
the action groupoid of $G_2$ on those states.

It sends a base arrow $f:x\to y$ to transport by the arrow element
$a_A(f)=a_A(y)\,\operatorname{cayleyProjective}(r_f)\,a_A(x)^{-1}$ of
\eqref{eq:orbit-stabilizer}. On a state this moves the input coordinate by the
input leg $\operatorname{cayleyProjective}(r_f)$ and the positioned coordinate
by the positioned leg, the two horizontal legs of the square whose verticals
are the frames at the two ends; on an arrow it is the identity on $\gamma$,
since $G_2$ commutes past the positioning. The identity and composition laws
hold because the fixed representatives force $r_{1_x}=1$ and
...
It sends a base object $X$ to the full subgroupoid of $\mathcal A_A(X)$ on the
witnessed states. An object there is a state $x$ of $\mathcal A_A(X)$ together
with a north residue state $x_N$ and a base arrow $g:N\to X$ satisfying
$\mathcal A_A(g)(x_N)=x$; the witness travels with the object. An arrow is an
arrow of $\mathcal A_A(X)$ between two such states, so a single element
$\gamma\in G_2$, as in the ambient fibre.

It sends a base arrow $f:X\to Y$ to the restriction of the ambient transport
$\mathcal A_A(f)$. That restriction lands in the subgroupoid because a
witnessed state stays witnessed, with the \emph{same} north state and the
composite witness $g\fatsemi f$: functoriality of $\mathcal A_A$ gives
