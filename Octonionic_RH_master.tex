...
\]
The matrix $C$ is the Cayley map $w\mapsto(w-i)/(w+i)$. Conjugating by it and
passing to the projective class is a single operation, used at every level of
what follows; write it
\[
  \operatorname{cayleyProjective}(g)=\bigl[C\,g\,C^{-1}\bigr],
\]
so that the slice M\"obius map attached to a logarithm is
\[
...
The states these functors act on form the \emph{normalized state world}
...
slice that direction selects; an arrow is a single element $\gamma\in G_2$,
moving the direction and carrying the coordinate with it. It has two functors
to $\mathcal H_1$, reading a state's coordinate before and after $A_{\OO}$ acts:
the input eye $\operatorname{In}_A$ and the output eye $\operatorname{Out}_A$.
