...
derivation. The preimage is the C-residue locus: each member is by definition a state on an
$S^6$ C-residue zero sphere, so the inclusion lands in exactly the image on which the
$G_2$-action is already transitive \textup{(}Theorem~\ref{thm:G2-S6}\textup{)}. Orbit
...
\textup{(}C4\textup{)} and single-orbit, and a nonempty action groupoid whose action reaches
