...
%          element; orbit--stabilizer carries it across the base, giving the
%          A-section functor A : B -> SphereWorld with object and arrow
%          actions determined together. Then T_A = Int_B A is formed. The
%          readout is pi_0(T_A) = colim_B(pi_0 . A). The colimit performs the
...
Orbit--stabilizer carries this one element across the base to the A-section functor
$A:\mathcal B\to\SphereWorld$, whose Grothendieck construction is the total
$\mathcal T_A=\int_{\mathcal B}A$; the residue-$\CC$ zero-spheres enter as its
\textup{(C3)--(C4)} outputs. The readout
$\pi_0(\mathcal T_A)\cong\operatorname{colim}_{\mathcal B}(\pi_0\circ A)$ collapses the section's
...
transformation across the whole continuum of slice directions, assembling the A-section functor
$A:\mathcal B\to\SphereWorld$ over the projective base $\mathcal B$. Its Grothendieck construction
$\mathcal T_A=\int_{\mathcal B}A$ gathers every sphere and every value-transport into one object, and
...
functor laws. This is the A-section functor
\[
  A:\mathcal B\longrightarrow\SphereWorld,
\]
written \texttt{sectionFunctor\,A} in the Lean source, its object and arrow actions determined
together. The winding, lift-uniqueness, tameness, crossing, welding, and GPV laws are readings
of this one action.
The objects of $A(b)$ are normalized analytic and sphere-world value states, and its arrows are
...
$\pi_0\circ A:\mathcal B\to\mathbf{Set}$ is the components diagram required by the
...
$A:\mathcal B\to\SphereWorld$, from the projective great-circle base $\mathcal B$ to the
sphere-world groupoid $\SphereWorld$, form exactly one colimit/component class, and that
singleton class
...
algebra; this is the A-section functor $A:\mathcal B\to\SphereWorld$. Its normalized
states carry their real-value data and its arrows are the actual value transports.

Form $P_A=\pi_0\circ A$ and $\mathcal T_A=\int_{\mathcal B}A$. By
...
$A$ \cite[p.~102, proof of Lemma~8.3.4]{Riehl}. The A-section functor is global on the
...
viewpoint; the A-section functor gathers all of them over the base, the total object holds them
