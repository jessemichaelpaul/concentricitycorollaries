...
\[
  D_A=D(\lambda_A)
     =\operatorname{cayleyProjective}\bigl(M(\lambda_A)\bigr),
  \qquad
  M(\lambda_A)=\operatorname{diag}(e^{\lambda_A},1).
\]
That $\lambda_A$ is a \emph{logarithm} and not merely a number is what the GPV
facts of Part~2 supply, and it is a fact about slice-preserving exponentials
rather than an extra choice here. The slice exponential is a covering onto the
nonzero slice values \textup{(}Theorem~\ref{thm:slice-exp}\textup{)}, its
logarithm manifold carries the branches
\textup{(}Theorem~\ref{thm:log-manifold}\textup{)}, and a tame lift along a path
is unique once its initial rung is fixed, with the ambiguity between branches
exactly the winding rung in $2\pi i\ZZ$
\textup{(}Theorem~\ref{thm:winding-lift},
Proposition~\ref{prop:winding-signature}\textup{)}. Since
$e^{\lambda_A+2\pi i k}=e^{\lambda_A}$, every branch gives the same
$M(\lambda_A)$ and hence the same $D_A$: the logarithm is what carries the
winding, and the element it generates does not see it. This is why the tape can
run through the whole base while the distinguished element stays one element,
and it is recorded again below as \textup{(B6)--(B8)}.
