...
(Definition~\ref{def:slices}; \cite[\S1--\S2]{SeriesExp}), and the compactification adjoins the single
...
\cite[\S11]{SeriesExp})} --- with the following four properties.
...
Through the commuting triangle $\pi\circ E=\exp$ (\cite[Rem.~5.2(a)]{VS}), the unique tame
lift traverses the logarithm manifold as a single closed loop (\cite[Cor.~5.13]{GPVwind}),
and every point of the degenerate fibre it meets is, by Lemma~\ref{lem:exp-degenerate}, the
level $\log r$ paired with an odd winding height $I(2k+1)\pi$: all multiplicity in the fibre
lies in the winding direction (\cite[Cor.~5.21]{GPVwind}), none in the level. Since
