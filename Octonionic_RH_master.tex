...
A nontrivial zero of the zeta function, seen from the complex line alone, is a lone point in a strip. Seen from the compactified octonions, that same zero opens into a round six-dimensional sphere, and the critical-line question becomes a question of arrangement: are these spheres concentric, gathered about one shared real centre? No single eye can settle that. It takes the whole continuum of imaginary directions in the octonions, read together, to see whether the alignment holds.
...
\subsection*{The two geometric registers}
...
\uses{def:A-section, def:base, lem:residue-spheres, lem:pi0-grothendieck,
thm:winding-lift, prop:winding-signature, thm:identity}
Let $A$ be an $A$-section. The C-residue values transported by the A-section functor
...
\end{theorem}
\begin{proof}
Conditions \textup{C1--C3} determine one distinguished element: the Euler product presents it
at $0$ and the Weierstrass product at $N$, the two boundary points it fixes, as a single
diagonal M\"obius element, through the commuting triangle $\pi\circ E=\exp$; per prime a
centered degenerate family with one real level, each lifting uniquely to $N$. The winding,
lift-uniqueness, tameness, crossing, welding, and GPV transport laws are readings of that one
action. Orbit--stabilizer carries the element across the base by M\"obius group laws, its
object and arrow actions determined together and its identity and composition laws pure group
algebra. The resulting functor
$A^{\mathrm{slice}}:\mathcal B\to\SphereWorld$ is the geometric projection.
The $G_2$-equivariant realization on $\Ostar$ lifts this same action to the
normalized value-state fibres, giving
$\mathcal A_A:\mathcal B\to\mathbf{Grpd}$. Its normalized states carry their
real-value data and its arrows are the actual value transports.

Form $P_A=\pi_0\circ\mathcal A_A$ and
$\mathcal T_A=\int_{\mathcal B}\mathcal A_A$. By
Lemma~\ref{lem:pi0-grothendieck},
\[
  \pi_0(\mathcal T_A)\cong
  \operatorname*{colim}_{\mathcal B}P_A.
\]
Riehl's category-of-elements calculation shows that this colimit imposes exactly the
identifications generated by the maps of $P_A$, hence by the completed value transports of
$\mathcal A_A$ \cite[p.~102, proof of Lemma~8.3.4]{Riehl}. The A-section functor is global on the
continuum of sphere worlds and its analytic cargo is carried through the one common witness
$N$. The colimit therefore consumes the entire transport system at once. Condition
\textup{C4} supplies the nonempty infinite output population, and the completed transports
supply its categorical connectedness. Riehl's singleton criterion then identifies its
$\pi_0$ with a singleton \cite[Rem.~8.3.5]{Riehl}. Thus the populated residue-$\CC$ part of the
colimit is one class.

This categorical class and its carried real value are kept in distinct registers:
\[
  \kappa\in\pi_0(\mathcal T_A),\qquad c\in\RR.
\]
The first is the unique transport class; the second is the one real value already present in
the normalized states identified in that class. Therefore the colimit output is the one
real-value singleton $\{c\}$. The populated objects supplied by \textup{C3--C4} are precisely
the residue-$\CC$ zero $6$-sphere states. They arrived as outputs of the degenerate fibre,
never as input assumptions. Each therefore has centre $c$, and the spheres are concentric.
\end{proof}
