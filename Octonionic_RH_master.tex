...
this total. Transitivity is applied first: connected components consume all
morphism identifications and give
$\pi_0(\int_{\mathcal B}\mathcal R_A)=\{\kappa\}$. Only after this collapse is
the inherited component read
$\overline L_A:\pi_0(\int_{\mathcal B}\mathcal R_A)\to\RR$ applied to the sole
class $\kappa$. Writing $c=\overline L_A(\kappa)$, its certified evaluation on
the residue states says that every residue-$\CC$ zero $6$-sphere has centre
$c$. Thus the readout has the exact order
$J_A\to\pi_0(J_A)=\{\kappa\}\to\RR$. The theorem asserts
...
produced by \textup{C3--C4}. The total has its canonical projection to the
base, $(b,x)\mapsto b$ and
...
on by the A-specific transport functor. The components diagram
...
connected, so $\pi_0(J_A)=\{\kappa\}$. Only after that collapse, the inherited
map $\overline L_A:\pi_0(J_A)\to\RR$ is applied to $\kappa$. Its evaluation
...
structure supplies the post-component map and its state-evaluation theorem.
...
infinite.  In the fibre at $p_A$, a C-residue zero-sphere state satisfies
the following condition: its decoded positioned action coordinate
\[
 C^{-1}\mathbin{\cdot}
...
residue diagram is an A-specific transport of such a state.  The
...
if $z_n$ belongs to \eqref{eq:semantic-c-residue-locus}, the zero-sphere state
in the fibre at $p_A$ has positioned action coordinate $C\cdot z_n$, so
decoding presents the original analytic zero $z_n$ to $A_{\OO}$.
Condition \textup{C1} continues
...
states in the fibre at $p_A$, and the action of $\mathcal A_A$ supplies
...
x_0=\Bigl((I_1,u_1),\
 ...
 A_{\OO}\bigl(I_1,C^{-1}\mathbin{\cdot}
   (A^{\mathrm{slice}}(p_A)\mathbin{\cdot}u_1)\bigr)\Bigr).
                                      \tag{R}\label{eq:residue-pole-fibre-triple}
\]
...
C-residue zero-sphere state in the fibre at $p_A$, with analytic zero
coordinate $z_m$ in
...
$\operatorname{Im}w_1\ne0$ and $\operatorname{Im}w_Q\ne0$.  The upper-triangular
real projective matrix
\[
...
\]
has determinant $\alpha\ne0$, so it represents an element of
$\mathrm{PGL}(2,\RR)$. Its action $w\mapsto\alpha w+\beta$ sends
$w_1$ to $w_Q$.  Its Cayley conjugate therefore
...
restriction is made. The unique GPV fibre and its real structure belong to
those constructions. They supply the inherited component map and its
evaluation on residue states. The final readout uses that map only after
transitivity has collapsed the residue total under $\pi_0$.
