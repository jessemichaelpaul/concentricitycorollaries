...
$g^{-1}\mathbin{\gg}k\mathbin{\gg}h$ gives an ambient Grothendieck morphism $P\to Q$, and the
...
\[
  \operatorname{Out}_A
  =\operatorname{In}_A\mathbin{\gg}A_{\OO}
\]
...
$0$-to-$N$ tape.  That datum is nonzero, and this is what makes the positioned
action a group action.  The Weierstrass divisor is exact at $N$ --- the point at
which the two presentations are the same function --- and \textup{C1} continues
$g_A$ through the simple pole with $u_A=g_A(p_A)\neq0$
\textup{(}Definition~\ref{def:base}\textup{)}.  So
$\operatorname{diag}(u_A,1)$ has determinant $u_A\neq0$, its class lies in the
projective group, and $D_A$ is its conjugate by the Cayley map: invertible, with
each positioned frame $a_A(b)=C(o_b)D_A$ a group element.  Orbit--stabilizer
therefore applies to it.  There is moreover one $N$ here and not three: the
...
The full functor $F_A=\mathcal A_A$ joins these views in one constrained graph:
at $N$ it records
\[
  (I,u)\longmapsto
...
\[
  u_i=D_A^{-1}(z_i),\qquad D_A(u_i)=z_i.              \tag{P}
\]
...
$z=D_A(u)$ is the positioned coordinate, and the third entry is its realized
A-value.  On arrows, conjugation by $D_A$ turns an input leg $C(r)$ into the
positioned leg $D_A C(r)D_A^{-1}$, while $G_2$ carries the slice direction.
The coordinate comparison, the sphere normalization, and the pole-generated
action are consequently three readings of a single $F_A$-state.
...
We now read the two boundary faces as commuting action squares.  Let $S$ be the
common source frame of the fixed tape and let $D$ be its north frame.  Write
$L_E,L_W$ for the positioned legs of the Euler and Weierstrass faces.  Their
input legs are the Cayley actions of the uniquely determined stabilizer parts,
\[
  R_E=C(r_E),\qquad R_W=C(r_W),
\]
and the two square identities are
\[
  L_E S=D R_E,\qquad L_W S=D R_W.                 \tag{S}\label{eq:north-face-squares}
\]
Thus $F_A(E_N)$ and $F_A(W_N)$ carry both horizontal legs of the two
rectangles together with these equations.  The C3 boundary readings select a
single input $u_\ast$ on the common source tape and identify the corresponding
positioned outputs as
\[
  L_E\bigl(S(u_\ast)\bigr)=z_1=D(u_1),\qquad
  L_W\bigl(S(u_\ast)\bigr)=z_2=D(u_2).             \tag{B}\label{eq:north-boundary-readings}
\]
The two faces may therefore be displayed as
\[
\begin{tikzcd}[column sep=large,row sep=large]
u_\ast \arrow[r,"R_E=C(r_E)"] \arrow[d,"S"']&
u_1 \arrow[d,"D"]&
u_\ast \arrow[r,"R_W=C(r_W)"] \arrow[d,"S"']&
u_2 \arrow[d,"D"]\\
S(u_\ast) \arrow[r,"L_E"']&
z_1&
S(u_\ast) \arrow[r,"L_W"']&
z_2 .
\end{tikzcd}
\]
Evaluating \eqref{eq:north-face-squares} at $u_\ast$ and then using
\eqref{eq:north-boundary-readings} gives
\[
\begin{aligned}
 D\bigl(C(r_E)(u_\ast)\bigr)
   &=L_E\bigl(S(u_\ast)\bigr)=D(u_1),\\
 D\bigl(C(r_W)(u_\ast)\bigr)
   &=L_W\bigl(S(u_\ast)\bigr)=D(u_2).
\end{aligned}
\]
The north frame $D$ is a M\"obius automorphism, so cancellation by $D^{-1}$
yields the two input equations
\[
  C(r_E)(u_\ast)=u_1,\qquad C(r_W)(u_\ast)=u_2.    \tag{I}\label{eq:north-input-readings}
\]
...
$u_1$ to $u_2$; conjugating by $D$ says that its positioned leg carries
$z_1$ to $z_2$.
...
$F_A\bigl((g^{-1}\mathbin{\gg}k)\mathbin{\gg}h\bigr)
...
$x_N$ and the composite witness $g\mathbin{\gg}f:N\to Y$, because
\[
\begin{aligned}
  F_A(g\mathbin{\gg}f)(x_N)
...
carries the residue witness $g\mathbin{\gg}f$ established in \textup{(C)}.  Hence
\[
  \mathcal R_A(f)\mathbin{\gg}\iota_{A,Y}
  =\iota_{A,X}\mathbin{\gg}F_A(f).                    \tag{Inc}
\]
