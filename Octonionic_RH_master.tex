...
\item \textbf{On objects.} A base object $b$ is sent to the groupoid of
  normalized A-section states positioned by its object frame $a_A(b)=C(o_b)D_A$.
...
\item \textbf{On arrows.} A base arrow $f:x\to y$ is sent to transport by its
  arrow element $a_A(f)=a_A(y)\,C(r_f)\,a_A(x)^{-1}$ of
  \eqref{eq:orbit-stabilizer}.  On a state this moves the input coordinate by
  the input leg $C(r_f)$ and the positioned coordinate by the positioned leg,
  ...
  $r_{1_x}=1$ and $r_{f\fatsemi g}=r_g r_f$ carried by the homomorphism $C(-)$,
...
\item \textbf{On objects.} A base object $X$ is sent to the full subgroupoid of
  ...
  $F_A(g)(x_N)=x$.  The witness travels with the object.
\item \textbf{On arrows.} A base arrow $f:X\to Y$ is sent to the restriction of
  the ambient transport $F_A(f)$.  It lands, because a witnessed state stays
...
object $N$. The inverse-image construction above has already fixed the unique tame GPV
$0$-to-$N$ tape and its unique orbit--stabilizer factorization; each zero index selects a state
along that fixed base transport.
...
invertible.  Each object frame $a_A(b)=C(o_b)D_A$ is then a group element as
...
The full functor $F_A=\mathcal A_A$ is the A-section action functor.  It joins
these views in one constrained graph: at $N$ it records
\[
  (I,u)\longmapsto
...
\]
What $F_A$ does is sweep the slice.  The direction $I$ ranges over $S^6$ and
...
objects, $F_A$ inherits the frame $a_A(b)=C(o_b)D_A$; on arrows, it inherits
...
by the homomorphism $C(-)$.  This is why every rectangle later in the proof
...
$f:x\to y$ with input leg $C(r_f)$ as $a_A(f)=a_A(y)\,C(r_f)\,a_A(x)^{-1}$,
while $G_2$ carries the slice direction.  At a loop on $N$ the frames at both
ends are $a_A(N)=D_A$, and this reads as conjugation,
$a_A(f)=D_A\,C(r_f)\,D_A^{-1}$; that is the form the relative loop takes at the
...
inputs lie in the C-residue zero locus.  One square apiece, the same square.

The rectangles used below are therefore the functorial arrows $F_A(k_{EN,1})$ and
$F_A(k_{EN,2})$, both running on the one fixed tape from the common projective zero
...
\[
  r_1=\operatorname{stabilizerPart}(k_{EN,1}),\qquad
  r_2=\operatorname{stabilizerPart}(k_{EN,2}).
\]
These residual factors are forced.  Orbit--stabilizer gives
$k_{EN,i}=o_N\,r_i\,o_0^{-1}$, and \texttt{stabilizerPart\_unique} says
...
\[
  R_1=C(r_1),\qquad R_2=C(r_2);
\]
along the bottom the arrow elements $a_A(k_{EN,1})$ and $a_A(k_{EN,2})$.  Read at $x=0$,
$y=N$, \eqref{eq:orbit-stabilizer} says that each square commutes:
\[
  a_A(k_{EN,1})\,a_A(0)=D_A\,R_1,\qquad
  a_A(k_{EN,2})\,a_A(0)=D_A\,R_2.       \tag{S}\label{eq:north-face-squares}
\]
What $F_A(k_{EN,1})$ and $F_A(k_{EN,2})$ contribute is that they carry both horizontal
...
\[
  a_A(k_{EN,1})\mathbin{\cdot}\bigl(a_A(0)\mathbin{\cdot}u_\ast\bigr)=z_1,\qquad
  a_A(k_{EN,2})\mathbin{\cdot}\bigl(a_A(0)\mathbin{\cdot}u_\ast\bigr)=z_2,
...
u_\ast \arrow[r,"R_1=C(r_1)"] \arrow[d,"a_A(0)"']\&
u_1 \arrow[d,"D_A"]\\
a_A(0)\mathbin{\cdot}u_\ast \arrow[r,"a_A(k_{EN,1})"']\&
z_1
\end{tikzcd}
\qquad\qquad
\begin{tikzcd}[column sep=6em,row sep=4em,ampersand replacement=\&]
u_\ast \arrow[r,"R_2=C(r_2)"] \arrow[d,"a_A(0)"']\&
u_2 \arrow[d,"D_A"]\\
a_A(0)\mathbin{\cdot}u_\ast \arrow[r,"a_A(k_{EN,2})"']\&
z_2
\end{tikzcd}
\]
...
\[
\begin{aligned}
 D_A\mathbin{\cdot}\bigl(C(r_1)\mathbin{\cdot}u_\ast\bigr)
   &=a_A(k_{EN,1})\mathbin{\cdot}\bigl(a_A(0)\mathbin{\cdot}u_\ast\bigr)
    =z_1=D_A\mathbin{\cdot}u_1,\\
 D_A\mathbin{\cdot}\bigl(C(r_2)\mathbin{\cdot}u_\ast\bigr)
   &=a_A(k_{EN,2})\mathbin{\cdot}\bigl(a_A(0)\mathbin{\cdot}u_\ast\bigr)
...
two input equations
\[
  C(r_1)\mathbin{\cdot}u_\ast=u_1,\qquad
  C(r_2)\mathbin{\cdot}u_\ast=u_2.    \tag{I}\label{eq:north-input-readings}
\]
...
\[
\begin{aligned}
 C(r)\mathbin{\cdot}u_1
 &=C(r_2r_1^{-1})\,C(r_1)\mathbin{\cdot}u_\ast\\
 &=C(r_2)\mathbin{\cdot}u_\ast=u_2,
\end{aligned}
\qquad
\begin{aligned}
 D_A\,C(r)\,D_A^{-1}\mathbin{\cdot}z_1
 &=D_A\,C(r)\mathbin{\cdot}u_1\\
 &=D_A\mathbin{\cdot}u_2=z_2 .
\end{aligned}
\]
The corresponding loop in the projective base is
\[
  k=k_{EN,1}^{-1}\fatsemi k_{EN,2}:N\longrightarrow N.
\]
...
\[
  ...
  =\operatorname{stabilizerPart}(k_{EN,2})
     \operatorname{stabilizerPart}(k_{EN,1})^{-1}
  =r_2r_1^{-1}=r.                                  \tag{R}\label{eq:north-relative-stabilizer}
\]
--- equivalently, from the unique factorizations $k_{EN,i}=o_Nr_i o_0^{-1}$,
...
\eqref{eq:north-relative-stabilizer} say that the input leg of $F_A(k)$ carries
...
$\gamma I_1=I_2$.  This is the step the ambient total cannot take.  $F_A$ sweeps
...
and identifications
\[
  F_A(g)(x_N)=P.\mathrm{fiber},\qquad
  F_A(h)(y_N)=Q.\mathrm{fiber}.                       \tag{N}\label{eq:north-witness-readings}
\]
...
\]
Recall that a morphism $P\to Q$ of $\int_{\mathcal B}F_A$ is a base arrow
$f:P.\mathrm{base}\to Q.\mathrm{base}$ together with a fibre arrow
$F_A(f)(P.\mathrm{fiber})\to Q.\mathrm{fiber}$.  Functoriality gives
$F_A\bigl((g^{-1}\fatsemi k)\fatsemi h\bigr)
 =F_A(h)\circ F_A(k)\circ F_A(g^{-1})$, and
$F_A(g^{-1})(P.\mathrm{fiber})=x_N$ by \eqref{eq:north-witness-readings}.
...
finally $F_A(h)$ carries that arrow forward to $Q.\mathrm{fiber}$, with
...
Let $f:X\to Y$.  If $x$ has witness \textup{(W)}, then $F_A(f)(x)$ has the same north state
$x_N$ and the composite witness $g\fatsemi f:N\to Y$, because
\[
\begin{aligned}
  F_A(g\fatsemi f)(x_N)
    &=F_A(f)\bigl(F_A(g)(x_N)\bigr)\\
    &=F_A(f)(x).
\end{aligned}                                         \tag{C}
\]
...
transport $F_A(f)$.  Composition in the projective base therefore supplies closure of the
...
\mathcal A_A(X) \arrow[r,"F_A(f)"'] & \mathcal A_A(Y).
\end{tikzcd}                                           \tag{Nat}
\]
For an object $x$, both routes in \textup{(Nat)} have underlying state $F_A(f)(x)$; for an arrow
$\alpha$, both routes have underlying arrow $F_A(f)(\alpha)$.  The upper route additionally
carries the residue witness $g\fatsemi f$ established in \textup{(C)}.  Hence
\[
  \mathcal R_A(f)\fatsemi \iota_{A,Y}
  =\iota_{A,X}\fatsemi F_A(f).                    \tag{Inc}
\]
