...
$\{\, m : M \mid m \cdot p = q \,\}$
\textup{(}\texttt{CategoryTheory.\-Action\-Category.\-hom\_as\_subtype}\textup{)}. In a Grothendieck
construction, an arrow \emph{is} a pair --- a base channel together with the compatible fibre
transport over it \textup{(}\texttt{CategoryTheory.\-Grothendieck.\-Hom}\textup{)}. Read on the
total, an arrow of $\int_{\mathcal B}\mathcal R_A$ is one element of the distinguished action,
and the pair is how that element presents itself, by its base part and its value-transport
part.
\[
\begin{tikzcd}[column sep=huge]
(X,\,x) \arrow[r, "{(f,\;\varphi_f)}"] & (Y,\,y)
\end{tikzcd}
\]
