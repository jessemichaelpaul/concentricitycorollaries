...
the element at every footpoint of the base, $G_2$ carries every direction to every other
\textup{(}Theorem~\ref{thm:G2-S6} --- the transitivity already in hand, proved for the sphere
images the selection is cut from\textup{)}, and membership travels by composition, so the
residue states form a single orbit under the action that produced them. A category is connected when its objects are joined by finite zigzags of
