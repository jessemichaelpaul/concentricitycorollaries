...
other --- and the action reaches every member of the preimage: orbit representatives position
the element at every footpoint of the base, $G_2$ carries every direction to every other, and
membership travels by composition, so the residue states form a single orbit under the action
that produced them. A category is connected when its objects are joined by finite zigzags of
...
\begin{remark}[An alternative route]\label{rmk:thomason}
Another proof of Theorem~\ref{thm:concentricity} is available through Thomason's homotopy
colimit theorem \cite{Thomason79}: the classifying space of a Grothendieck construction is
homotopy equivalent to the homotopy colimit of the classifying spaces of its fibres, and
reading connected components through that equivalence yields the same collapse of
$\pi_0\bigl(\int_{\mathcal B}\mathcal R_A\bigr)$. The proof given here stays at the level of
categories, where Lemma~\ref{lem:pi0-grothendieck} suffices; the Thomason route is the
topological shadow of the same computation, and that path has not been formalized in Lean.
\end{remark}
