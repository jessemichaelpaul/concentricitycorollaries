...
$g^{-1}\mathbin{;}k\mathbin{;}h$ gives an ambient Grothendieck morphism $P\to Q$, and the
...
\[
  \operatorname{Out}_A
  =\operatorname{In}_A\mathbin{;}A_{\OO}
\]
...
\]
The common pole datum is therefore read directly in the north action, with
Euler and Weierstrass giving its two boundary presentations along the same
$0$-to-$N$ tape.  Two facts about $N$ make that datum nonzero, and they are what
puts \eqref{eq:orbit-stabilizer} at our disposal here.  First, the Weierstrass
divisor is exact at $N$: that is the one point at which the two presentations
are the same function, so the datum they share is read off a single value there
rather than compared across two.  Second, \textup{C1} continues $g_A$
\emph{through} the simple pole, so that value exists and is
...
\[
  ...
  \bigl((I,u),\;D_A\mathbin{\cdot}(I,u),\;
    A_{\OO}(D_A\mathbin{\cdot}(I,u))\bigr).          \tag{A}
\]
...
two forced identities $r_{1_x}=1$ and $r_{f\mathbin{;}g}=r_g r_f$ transported
...
normalized north frame.  The rectangles used below are the functorial arrows
$F_A(E_N)$ and $F_A(W_N)$, where $E_N$ and $W_N$ are the north Euler and
Weierstrass boundary actions.  Both run on the one fixed tape from the common
projective zero frame to the one north frame.  Write
\[
...
\]
These residual factors are forced. For each of the two boundary arrows,
orbit--stabilizer gives
\[
  k_\bullet=o_N\,r_\bullet\,o_0^{-1},
\]
and \texttt{stabilizerPart\_unique} says that any north-stabilizer element giving this same
factorization equals $r_\bullet$. Hence the Euler and Weierstrass faces are the two boundary
presentations on the one fixed $0$-to-$N$ transport, with uniquely determined stabilizer parts.

We now read the two boundary faces as commuting action squares.  Their vertical
arrows are the object frames: $a_A(0)$ at the source of the fixed tape, and
$a_A(N)=D_A$ at its north frame.  Their horizontal arrows are the two legs of
\eqref{eq:orbit-stabilizer}.  On top, the input legs are the Cayley actions of
the uniquely determined stabilizer parts,
\[
  R_E=C(r_E),\qquad R_W=C(r_W);
\]
along the bottom, the positioned legs are the arrow elements $a_A(k_E)$ and
$a_A(k_W)$.  Read at $x=0$, $y=N$, \eqref{eq:orbit-stabilizer} says that each
square commutes:
\[
  a_A(k_E)\,a_A(0)=D_A\,R_E,\qquad
  a_A(k_W)\,a_A(0)=D_A\,R_W.       \tag{S}\label{eq:north-face-squares}
\]
What $F_A(E_N)$ and $F_A(W_N)$ contribute is that they carry both horizontal
legs together with these equations.

The fixed tape supplies the input at which the two faces are read: they leave
the same source object, so they share its normalized input coordinate, which we
write $u_\ast$.  The C3 boundary readings identify the corresponding positioned
outputs with the authored coordinates,
\[
  a_A(k_E)\mathbin{\cdot}\bigl(a_A(0)\mathbin{\cdot}u_\ast\bigr)=z_1,\qquad
  a_A(k_W)\mathbin{\cdot}\bigl(a_A(0)\mathbin{\cdot}u_\ast\bigr)=z_2,
                                   \tag{B}\label{eq:north-boundary-readings}
\]
and \eqref{eq:north-preimages} rewrites each authored coordinate as
$z_i=D_A\mathbin{\cdot}u_i$.  The two faces may therefore be displayed as
\[
\begin{tikzcd}[column sep=large,row sep=large]
u_\ast \arrow[r,"R_E=C(r_E)"] \arrow[d,"a_A(0)"']&
u_1 \arrow[d,"D_A"]&
u_\ast \arrow[r,"R_W=C(r_W)"] \arrow[d,"a_A(0)"']&
u_2 \arrow[d,"D_A"]\\
a_A(0)\mathbin{\cdot}u_\ast \arrow[r,"a_A(k_E)"']&
z_1&
a_A(0)\mathbin{\cdot}u_\ast \arrow[r,"a_A(k_W)"']&
z_2 .
\end{tikzcd}
\]
...
\[
\begin{aligned}
 D_A\mathbin{\cdot}\bigl(C(r_E)\mathbin{\cdot}u_\ast\bigr)
   &=a_A(k_E)\mathbin{\cdot}\bigl(a_A(0)\mathbin{\cdot}u_\ast\bigr)
    =z_1=D_A\mathbin{\cdot}u_1,\\
 D_A\mathbin{\cdot}\bigl(C(r_W)\mathbin{\cdot}u_\ast\bigr)
   &=a_A(k_W)\mathbin{\cdot}\bigl(a_A(0)\mathbin{\cdot}u_\ast\bigr)
...
two input equations
\[
  C(r_E)\mathbin{\cdot}u_\ast=u_1,\qquad
  C(r_W)\mathbin{\cdot}u_\ast=u_2.    \tag{I}\label{eq:north-input-readings}
\]
This is the precise passage from the two functorial boundary faces to their
values at the common input.  The north comparison now follows by algebraic
cancellation. Put
\[
  r=r_Wr_E^{-1}.
\]
The required comparison is the elementary group calculation for these two
boundary faces of the distinguished action:
\[
\begin{aligned}
 C(r)\mathbin{\cdot}u_1
 &=C(r_Wr_E^{-1})\,C(r_E)\mathbin{\cdot}u_\ast\\
 &=C(r_W)\mathbin{\cdot}u_\ast=u_2,
\end{aligned}
\qquad
\begin{aligned}
...
\[
  k=k_E^{-1}\mathbin{;}k_W:N\longrightarrow N.
\]
...
\[
  ...
  =\operatorname{stabilizerPart}(k_W)
     \operatorname{stabilizerPart}(k_E)^{-1}
  =r_Wr_E^{-1}=r.                                  \tag{R}\label{eq:north-relative-stabilizer}
\]
The same identity can be seen directly from the unique factorizations
$k_\bullet=o_Nr_\bullet o_0^{-1}$: the inverse Euler face followed by the
Weierstrass face cancels the common $0$-frame $o_0$, and $o_N=1$ at the north
frame.  Equations \eqref{eq:north-input-readings} and
\eqref{eq:north-relative-stabilizer} say that the input leg of $F_A(k)$ carries
$u_1$ to $u_2$; conjugating by $D_A$ says that its positioned leg carries
$z_1$ to $z_2$.

The two north states also carry directions $I_1,I_2\in S^6$.  Transitivity
of the $G_2$-action supplies $\gamma\in G_2$ with
$\gamma I_1=I_2$.  The equality of the transported coordinates and this
direction action are precisely the two components of an arrow
\[
...
$F_A\bigl((g^{-1}\mathbin{;}k)\mathbin{;}h\bigr)
...
$x_N$ and the composite witness $g\mathbin{;}f:N\to Y$, because
\[
\begin{aligned}
  F_A(g\mathbin{;}f)(x_N)
...
carries the residue witness $g\mathbin{;}f$ established in \textup{(C)}.  Hence
\[
  \mathcal R_A(f)\mathbin{;}\iota_{A,Y}
  =\iota_{A,X}\mathbin{;}F_A(f).                    \tag{Inc}
\]
