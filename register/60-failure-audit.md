...
| 26 | **CORRECTED 2026-07-28 (author's ruling, positive register).** `∫𝓡_A` is exhibited as a literal `ActionCategory` and IS CONNECTED from the fact — ALREADY GREEN — that `ι_A` is a natural inclusion and iso onto its image; `Action.lean:128` fires (pretransitivity = one orbit, `Nonempty` = certified inhabitants). The old row banned this route by freezing a grep into a prohibition | kernel-test, never grep-freeze |
